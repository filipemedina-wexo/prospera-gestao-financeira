-- Fix 1: Correct status mapping and create proper enum handling
...
-- Fix 2: Update RPC function to handle proper status mapping
...
-- Fix 3: Update financial transaction triggers to use correct status values
...
-- Fix 4: Recreate triggers with proper names
...
-- Fix 5: Ensure all existing data has correct status values
-- Update any invalid statuses in accounts_receivable
UPDATE public.accounts_receivable 
SET status = 'pending' 
WHERE status NOT IN ('pending', 'received', 'overdue', 'partial', 'paid');
...
-- Fix 6: Ensure category mappings exist for all accounts
UPDATE public.accounts_payable 
SET category_id = (
